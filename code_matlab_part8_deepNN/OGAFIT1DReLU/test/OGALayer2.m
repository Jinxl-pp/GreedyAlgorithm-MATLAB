function [errL2, parameters, uk] = OGALayer2(paramLayer1, nNeuron, degree, pde, interval, quadInterval)
...
W2Layer1 = paramLayer1(:,1);
W1Layer1 = paramLayer1(:,2);
BiasLayer1 = paramLayer1(:,3);
inputDim = size(W2Layer1,1);
auxInfo.dim = inputDim;
layer1 = @(x)(activation(W1Layer1*x+BiasLayer1)); 
auxInfo.layer1 = layer1;

%% largest value of L2 norm of neurons from last layer
x = -1:0.001:1;
c = activation(W1Layer1*x+BiasLayer1);
c = max(sqrt(sum(c.^2,1)));
...
W2 = zeros(nNeuron-2,1);        % w2
W1 = zeros(nNeuron-2,inputDim); % w1
Bias = zeros(nNeuron-2,1);      % bias
C = zeros(nNeuron-2,numpts);
...
for k = 1 : nNeuron-2
    ...
    start_time = clock;
    square = [-c-1,c+1];
    theta = optimalBasisLayer2(xqd,wei,square,auxInfo,h);
    ...
    uk = @(x)(cg*activation(w*layer1(x)+b)); 
...
%% add two nodes using the relation x = ReLU(x)-ReLU(-x)
W2 = [W2; 1; -1];
W1 = [W1; W2Layer1'; -W2Layer1'];
Bias = [Bias; 0; 0];
clear W2Layer1 W1Layer1 BiasLayer1 paramLayer1

%% outputs
parameters  = [W2, W1, Bias];
uk = @(x)(W2'*activation(W1*layer1(x)+Bias)); % x has to be a row vector
