function [errL2, parameters, uk, Net] = ogaForLayer_uniform(Net, degree, pde, interval, quadInterval)
...
%% use uniform mesh for the first layer
if layer == 0
    W1 = W1 + 1;
    Bias = linspace(-0.9,1,trainingSteps)';
    ...
    %%%
    f = (pde.residual(xqd).*wei)';
    bk = g*f*(h/2);                           % f*gi
    %%%
    xk = Gk \ bk;
    W2 = xk;
    cg = xk';
    uk = @(x)(cg*activation(W1*Net.preLayer(x)+Bias)); 
